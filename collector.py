...
class ObservationSpace:
    def __init__(self, low, high, shape):
        self.low = torch.full(shape, low, dtype=torch.float32)
        self.high = torch.full(shape, high, dtype=torch.float32)
...
class World(object):
    ...
    def restart(self):
        ...
        if self._actor_filter == 'vehicle.tesla.model3':
        ...
        while self.player is None:
            ...
            spawn_point = self.map.get_spawn_points()[53] #random.choice(spawn_points) if spawn_points else carla.Transform()
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
        # Set up the sensors.
...
def main():
    ...
    argparser.add_argument(
        ...
        default='160x80',
        type=str,
        help='Observation resolution (default: 160x80)')
    argparser.add_argument(
        '--output_dir', 
        default='images', 
        type=str, 
